
# Sit L. 👷

**`I.T.`** **`Software Engineering`** **`Game Development`**

I am currently looking for internship or full-time employment opportunity in I.T. and software development only. I know how to program in C++, Java, Python and Lua. I like to develop games on my own free time :)
<p align="left">
  

<a href="https://devforum.roblox.com/u/DemonKingLosi">
  <img alt="MY RBX Dev" title="This is my Roblox Developer Profile" src="https://custom-icon-badges.demolab.com/badge/RBX-Developer-blue.svg?logo=rbxuno">
</a>

</p>

---
### 🛠️ Languages, Tools and Frameworks
<img align="left" alt="LUA" width="60px" style="padding-right:10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/lua/lua-original.svg"/>
<img align="left" alt="CPlusPlus" width="60px" style="padding-right:10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg"/>
<img align="left" alt="Python" width="60px" style="padding-right:10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/python/python-original-wordmark.svg"/>  
<img align="left" alt="Java" width="60px" style="padding-right:10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original-wordmark.svg"/>




#

<br>

<br>

### Stats 📊💡

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=cyberfriend6&show_icons=true&theme=tokyonight)

#

<details>
  <summary><h3>🧑 My Progress</summary>
    Hello! if you are reading this, it means you are interested in my story chasing a goal that I thought was once impossible. I struggled with learning how to code while being a full time college student and working. Eventually, I decided to take a much needed break from my academics that was due to not having a goal and went blindly into the computer science path without any guidance. Within that break, I have work in many occupations, gaining insights of different workfields that gave me knowledge and skills that can be applied towards I.T. and programming. The time I took time off gave me clarity and gave me time to contemplate and finally rediscover my goal for as a techie which helped me want to learn more in order to enter the tech field. I spent most of my education within Fresno City College, as I have graduated and received my A.S in Computer Information Systems learning IT fundamentals, computer networking, and programming. I will be ready to start at my local CSU to work on completing an undergrad for B.S. in Computer Science. It took me many years to realize I have been doing the same programming concept repetitively resulting me to start understanding programming as a whole while also learning new things. I am grateful where I am currently because without all these mistakes and errors I have come across, I would have not been pursuing a path into tech. Now, without even thinking, I believe I have become tech worker by heart and coding is engraved within me as I like to believe that this world runs like computers working through while, if and for loop statements to reach a certain action and output that will lead me to my career goal.
</details>


<!--
**Official3Lo/Official3Lo** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
